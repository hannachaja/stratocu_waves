...
prevWaveletSpec = [];
prevImage = [];
velocityFields = struct(); % Structure to store velocity fields
waveletFeatures = struct(); % Structure to store wavelet features
...
for f_idx = 1:4
    ...
    % Resize if needed.
    if shrinkfactor ~= 1
        data_pre = imresize(data_pre, 1/shrinkfactor);
    end
   ...
   % produceAnnotatedImages(instrument, spec_full, data_filt, squares, ...
    %    Scales, Angles, singleOutDir, frameDateStr, ...
...
%}
    ...
    % Calculate PIV between the current and previous frame
    if f_idx > 1
        velocityField = calculatePIV(prevImage, data_pre, shrinkfactor);
        velocityFields.(['frame_' frameDateStr]) = velocityField; % Store the velocity field in the structure with a valid field name
    end

     % Extract wavelet features
    waveletFeatures.(['frame_' frameDateStr]) = extractWaveletFeatures(instrument, spec_full, data_filt, squares, ...
        Scales, Angles, frameDateStr, ...
        nAngles_fineFactor, nScales_fineFactor, peakDetectionFactor, ArrayMode, contourArray);
    
    if ~isempty(prevWaveletSpec)
        % Calculate the average coherence map for each peak
        if isfield(waveletFeatures, ['frame_' frameDateStr])
            numPeaks = waveletFeatures.(['frame_' frameDateStr]).NumPeaks;
            for pk = 1:numPeaks
                peak = waveletFeatures.(['frame_' frameDateStr]).PeakRegions(pk);
                coherenceMap = zeros(size(spec_full, 1), size(spec_full, 2));
                phaseMap = zeros(size(spec_full, 1), size(spec_full, 2));
                numCoherences = 0;
    
                for jj = 1:numel(peak.ScaleIndices)
                    s_idx = peak.ScaleIndices(jj);
                    a_idx = peak.AngleIndices(jj);
                    coherence = spec_full(:,:,s_idx,a_idx);
                    coherenceNormalized = normalizeData(abs(coherence));
                    coherenceMap = coherenceMap + coherenceNormalized;
                    phaseMap = phaseMap + angle(coherence);
                    numCoherences = numCoherences + 1;
                end
    
                coherenceMapAvg = coherenceMap / numCoherences;
                phaseMapAvg = phaseMap / numCoherences;
    
                % Store the average coherence map and phase map for the current peak
                waveletFeatures.(['frame_' frameDateStr]).PeakRegions(pk).CoherenceMapAvg = coherenceMapAvg;
                waveletFeatures.(['frame_' frameDateStr]).PeakRegions(pk).PhaseMapAvg = phaseMapAvg;
            end
        end
    end


    % Update previous frame’s transform and image.
...
% Plot each coherence map after the main processing loop
for f_idx = 1:numFrames
    frameDateStr = datestr(fTimes(f_idx), 'yyyy_mm_dd_HHMMSS');
    if isfield(waveletFeatures, ['frame_' frameDateStr])
        numPeaks = waveletFeatures.(['frame_' frameDateStr]).NumPeaks;
        for pk = 1:numPeaks
            peak = waveletFeatures.(['frame_' frameDateStr]).PeakRegions(pk);
            if isfield(peak, 'CoherenceMapAvg')
                coherenceMapAvg = peak.CoherenceMapAvg;
                phaseMapAvg = peak.PhaseMapAvg;

                % Create a figure for the coherence map
                figure;
                imagesc(coherenceMapAvg);
                colormap('jet');
                clim([0 1]);
                colorbar;
                hold on;

                % Add quiver plot with larger and constant-sized arrows
                if isfield(velocityFields, ['frame_' frameDateStr])
                    velocityField = velocityFields.(['frame_' frameDateStr]);
                    xtable = velocityField(1:shrinkfactor:end, 1:shrinkfactor:end, 1);
                    ytable = velocityField(1:shrinkfactor:end, 1:shrinkfactor:end, 2);
                    utable = velocityField(1:shrinkfactor:end, 1:shrinkfactor:end, 3);
                    vtable = velocityField(1:shrinkfactor:end, 1:shrinkfactor:end, 4);

                    % Normalize the vectors to have constant length
                    norm_factor = 200; % Adjust this factor to control the arrow size
                    utable = utable / norm(utable(:)) * norm_factor;
                    vtable = vtable / norm(vtable(:)) * norm_factor;

                    quiver(xtable, ytable, utable, vtable, 'AutoScale', 'off', 'Color', 'm', 'LineWidth', 1.5, 'MaxHeadSize', 0.5);
                end

                % Add contour lines
                waveSum_up = zeros(size(data_filt));
                wavePower_up = zeros(size(data_filt));
                for jj = 1:numel(peak.ScaleIndices)
                    s_idx = peak.ScaleIndices(jj);
                    a_idx = peak.AngleIndices(jj);
                    coeff = spec_full(:,:,s_idx,a_idx);

                    % Resize coeff to match the size of waveSum_up
                    coeff_resized = imresize(real(coeff), size(waveSum_up));
                    waveSum_up = waveSum_up + coeff_resized;
                    wavePower_up = wavePower_up + abs(coeff_resized).^2;
                end

                % Downsample the contours
                waveSum_down = imresize(waveSum_up, 1/shrinkfactor);
                wavePower_down = imresize(wavePower_up, 1/shrinkfactor);

                % Choose contour levels based on either absolute values or percentiles.
                contourOption = 'percentile'; % or 'percentile'
                contourArray = [95 97 99]; % Example contour levels
                switch lower(contourOption)
                    case 'absolute'
                        contourLevels = contourArray;
                    case 'percentile'
                        contourLevels = prctile(abs(waveSum_down(:)), contourArray);
                    otherwise
                ...
                % Plot contours:
                % For positive values:
                contour(waveSum_down, contourLevels, 'LineColor', 'r', 'LineWidth', 0.5);
                % For negative values (mirror the levels):
                contour(waveSum_down, -contourLevels, 'LineColor', 'b', 'LineWidth', 0.5);

                % Calculate displacement vectors from phase
                effective_scale = mean(peak.MeanScale);
                wavelength_km = (pi * effective_scale * pixel_size_km) / sqrt(2);
                time_interval = seconds(fTimes(f_idx) - fTimes(f_idx-1));
                speed_phase = ((phaseMapAvg * wavelength_km) / (2*pi)) / time_interval * 1000; % in m/s

                % Moyenner les vitesses de phase
                speed_phase_mean = blockproc(speed_phase, size(utable), @(block_struct) mean(block_struct.data, 'all'));

                % Interpoler sur la grille PIV
                [X, Y] = meshgrid(1:size(speed_phase_mean, 2), 1:size(speed_phase_mean, 1));
                [Xq, Yq] = meshgrid(1:size(xtable, 2), 1:size(xtable, 1));
                speed_phase_interp = interp2(X, Y, speed_phase_mean, Xq, Yq, 'linear');

                % Plot phase-based displacement vectors
                [grad_x, grad_y] = gradient(speed_phase_interp);
                % Normalize the vectors to have constant length
                norm_factor = 100; % Adjust this factor to control the arrow size
                grad_x= grad_x / norm(grad_x(:)) * norm_factor;
                grad_y = grad_y / norm(grad_y(:)) * norm_factor;
                quiver(xtable, ytable, grad_x, grad_y, 'AutoScale', 'off', 'Color', 'k', 'LineWidth', 1.5, 'MaxHeadSize', 0.5);

                % Add peak information to the title
                scaleStr = sprintf('%.1f', peak.MeanScale);
                angleStr = sprintf('%.0f', rad2deg(peak.MeanAngle));
                subtitleText = sprintf('Scales: %s, Angles: %s°', scaleStr, angleStr);
                title({sprintf('Average Coherence Map for Peak %d - Frame: %s', pk, frameDateStr), subtitleText}, 'Interpreter', 'none');
                xlabel('X Position');
                ylabel('Y Position');
                hold off;
            end
