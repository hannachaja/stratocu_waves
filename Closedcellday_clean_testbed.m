...
raw    = red(1:1000,1:1450,3) ; % raw copy for backdrop
fframe = red(1:1000,1:1450,3) ; % 3rd image in video
fframe2= red(1:1000,1:1450,4) ; % 4th image in video, 1/2 h later
...
% Also consider compressing the dynamic range? 
fframe = preprocess_img(fframe);
fframe2= preprocess_img(fframe2);
...
figure(1)
imagesc(fx,fy,raw); colormap('gray'); colorbar; axis on; hold on; 
...
figure(2)
imagesc(fx,fy,fframe); colormap('turbo'); colorbar; axis on; hold on; 
title('preprocessed array')
...
% print information 
 ...
 % contours are -10 to 10 / clevfactor so make clev=ampli/3 or so 
    ...
    image_with_wavelet_overlay(raw, spec,x,y, Scales, isc,ian, clev); 
